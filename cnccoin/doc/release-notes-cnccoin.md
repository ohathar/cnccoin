CNCcoin Core version *0.15.1* is now available from:

  <https://download.cnccoin.org/cnccoin-0.15.1/>
...
A number of changes to the way CNCcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 beacons per kB (0.00001 CNC). This is to prepare for dropping the minimum transaction fee to 0.1 beacons per kB (0.0001 CNC) in the next release.
